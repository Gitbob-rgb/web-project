...
class EntrepriseModel {
    ...
    public function getAllEntreprises($page = 1, $perPage = 10) {
        $offset = ($page - 1) * $perPage;
        
        // Get total count
        $countQuery = "SELECT COUNT(*) as total FROM entreprises";
        $countStmt = $this->db->query($countQuery);
        $totalCount = $countStmt->fetch()['total'];
        
        // Get paginated results
        $query = "SELECT e.*, 
                 (SELECT COUNT(*) FROM offres_stage o WHERE o.entreprise_id = e.id) as offre_count,
                 (SELECT COUNT(*) FROM candidatures c JOIN offres_stage o ON c.offre_stage_id = o.id WHERE o.entreprise_id = e.id) as candidature_count,
                 IFNULL((SELECT AVG(note) FROM notations n WHERE n.entreprise_id = e.id), 0) as moyenne_notation
                 FROM entreprises e
                 ORDER BY e.nom ASC
